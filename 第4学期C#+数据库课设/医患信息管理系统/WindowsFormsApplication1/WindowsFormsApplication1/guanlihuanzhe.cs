...
{
    public partial class guanlihuanzhe : Form
    {
        ...
        public guanlihuanzhe()
        {
            InitializeComponent();
            gethuanzhe();
        }
        public void gethuanzhe()//显示全部患者信息
        {
            s.Open();
        ...
        public void baocun()
        {
            ...
            getmassage();
            s.Open();
            string chazhao = "select num from yy_huanzhe where num='"+num+"'";
            if (s.GetDataTable(chazhao).Rows.Count == 0)
            {
                string sql = "insert into yy_huanzhe(num,name,sex,age,telephone,guahao,keshihao,password) values('" + num + "','" + name + "','" + sex + "','" + age + "','" + telephone + "','" + guahao + "','" + keshihao + "','1')";//密码默认为1
                ...
                else
                {
                    ...
                    sql = "select num 编号,name 患者姓名,sex 性别,age 年龄,telephone 联系方式,guahao 挂号,keshihao 科室号 from yy_huanzhe where num='" + num + "'";
                    DataTable dt = s.GetDataTable(sql);
                    dataGridView1.DataSource = dt;
                }
            }
        ...
        void chaxun()//查询函数
        {
            getmassage();
            s.Open ();
            string sql = "select num 编号,name 患者姓名,sex 性别,age 年龄,telephone 联系方式,guahao 挂号,keshihao 科室号 from yy_huanzhe where password='1' ";//编写sql语句,通过一些if语句来实现输入信息越多,查询范围越少
        ...
        void xiugai()           //修改函数
        {
            ...
            if (num == yuanshinum)
            {
                string sql = "update yy_huanzhe set name='" + name + "',sex='" + sex + "',age='" + age + "',telephone='" + telephone + "',guahao='" + guahao + "',keshihao='" + keshihao + "' where num='"+num+"'" ;
                int n = s.ExecuteNonQuery(sql);
                if (n == 0)
                {
        ...
        void shanchu()        //删除函数
        {
            ...
            if (num == yuanshinum)
            {
                ...
                if (q.b)
                {
                    string sql = "delete from yy_huanzhe  where num='" + num + "'";
                    int n = s.ExecuteNonQuery(sql);
                    if (n == 0)
                    {
